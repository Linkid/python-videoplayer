...
if ($env:platform -eq "x64") {
    cmd.exe /c "call `"C:\Program Files\Microsoft SDKs\Windows\v7.1\Bin\SetEnv.cmd`" /x64"
    cmd.exe /c "call `"C:\Program Files (x86)\Microsoft Visual Studio 14.0\VC\vcvarsall.bat`" x86_amd64"
    cmd.exe /c "set > %temp%\vcvars.txt"
} else {
